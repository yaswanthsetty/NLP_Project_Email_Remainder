...
try:
    ...
    from notifier import send_desktop_notification, format_event_notification
    ...
    for i, message in enumerate(messages, 1):
        ...
        if actionable_events:
            ...
            for j, event in enumerate(actionable_events, 1):
                ...
                # Optional: Also send email reminder (if configured)
                # Uncomment and configure the following lines if you want email reminders too
                # recipient_email = os.getenv("REMINDER_EMAIL", parsed_email.get('to', ''))
                # if recipient_email:
                #     send_email_reminder(recipient_email, title, message)
        else:
